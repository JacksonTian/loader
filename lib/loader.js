...
Loader.prototype.dev = function (version) {
  var html = '';
  var i, l;
  var versionNumber = version ? '?v=' + version : '';
  for (i = 0, l = this.script.assets.length; i < l; i++) {
    html += '<script src="' + this.script.assets[i] + versionNumber + '"></script>\n';
  }
  for (i = 0, l = this.style.assets.length; i < l; i++) {
    html += '<link rel="stylesheet" href="' + this.style.assets[i] + versionNumber + '" media="all" />\n';
  }
 ...
 * 生成线上环境下的标签
 */
Loader.prototype.pro = function (version, CDNMap) {
  var html = '';
  var versionNumber = version ? '?v=' + version : '';
  if (this.script.min) {
    var scriptMin = CDNMap[this.script.min] || (this.script.min + versionNumber);
    html += '<script src="' + scriptMin + '"></script>\n';
  }
  if (this.style.min) {
    var styleMin = CDNMap[this.style.min] || (this.style.min + versionNumber);
    html += '<link rel="stylesheet" href="' + styleMin  + '" media="all" />\n';
  }
  // for (i = 0, l = this.asyncAssets.length; i < l; i++) {
  //   html += '<script>\n' +
  //       '(function () {\n' +
 ...
 * 根据环境和版本号去生成对应标签
 */
Loader.prototype.done = function (version, CDNMap) {
  CDNMap = CDNMap || {};
  return (process.env.NODE_ENV === "production") ? this.pro(version, CDNMap) : this.dev(version);
};
...
Loader.minify = function (basedir, arr, justCombo) {
  arr.forEach(function (item, index) {
    ...
    if (justCombo) {
      fs.writeFileSync(path.join(basedir, min), content);
    } else {

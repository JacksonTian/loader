...
/**
 * 根据环境和版本号去生成对应标签
 * 如果combo没有传入，将取`process.env.NODE_ENV`作为判断，`production`为产品环境，其余将采用原始版本
 */
Loader.prototype.done = function (CDNMap, prefix, combo) {
  CDNMap = CDNMap || {};
  var mode = combo !== undefined ? combo : process.env.NODE_ENV === "production";

...
Loader.prototype.dev = function (version) {
  ...
  for (i = 0, l = this.script.assets.length; i < l; i++) {
    html += '<script src="' + this.script.assets[i] + '?v=' + version + '"></script>\n';
  }
  for (i = 0, l = this.style.assets.length; i < l; i++) {
    html += '<link rel="stylesheet" href="' + this.style.assets[i] + '?v=' + version + '" media="all" />\n';
  }
...
Loader.prototype.pro = function (version, CDNMap) {
  var html = '';
  if (this.script.min) {
    var scriptMin = CDNMap[path.basename(this.script.min)] || (this.script.min + '?v=' + version);
    html += '<script src="' + scriptMin + '"></script>\n';
  }
  if (this.style.min) {
    var styleMin = CDNMap[path.basename(this.style.min)] || (this.style.min + '?v=' + version);

 ...
 * 根据环境和版本号去生成对应标签
 */
Loader.prototype.done = function (CDNMap, prefix) {
  CDNMap = CDNMap || {};
  return (process.env.NODE_ENV === "production") ? this.pro(CDNMap, prefix) : this.dev();
};
...
Loader.minify = function (basedir, arr, justCombo) {
  arr.forEach(function (item, index) {
    ...
    if (justCombo) {
      ...
      var minified = path.extname(item.hash) === ".js" ? Loader.transformScript(content) : Loader.transformStyle(content);
      fs.writeFileSync(path.join(basedir, item.hash), minified);
    }
  });
  return arr;
};

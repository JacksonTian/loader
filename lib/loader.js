...
var Loader = function (js, css) {
  ...
  // global defalut options
    // this.CDNMap = undefined;
    // this.prefix = undefined;
    // this.isMinify = undefined;

  for (var key in Loader.defaults) {
    if (Loader.defaults.hasOwnProperty(key)) {
      this[key] = Loader.defaults[key];
    }
  }
  // END global defalut options


};

Loader.setGlobal = function (options) {
...
Loader.prototype.done = function (CDNMap, prefix, env) {
  ...
  // mode
  var mode;
  if (env !== undefined) { // 可能传入 0
    mode = env;
  } else if (this.isMinify !== undefined) {
    mode = this.isMinify;
  } else {
